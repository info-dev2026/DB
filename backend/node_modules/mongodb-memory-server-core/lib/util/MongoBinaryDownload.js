...
class MongoBinaryDownload {
    ...
    async extract(mongoDBArchive) {
        ...
        const filename = path_1.default.basename(mongodbFullPath);
        ...
        const binaryChecksum = await (0, utils_1.md5FromFile)(mongodbFullPath);
        ...
        if (!(0, utils_1.isNullOrUndefined)(extractedChecksum) && extractedChecksum !== binaryChecksum) {
            throw new errors_1.GenericMMSError(`MongoBinaryDownload: the extracted binary at "${mongodbFullPath}" does not match what was extracted from "${mongoDBArchive}"!
...
(expected: "${extractedChecksum}", actual: "${binaryChecksum}")`);
        ...
        await fs_1.promises.writeFile(`${mongodbFullPath}.md5`, `${binaryChecksum} *${filename}\n`);
        ...
        await new Promise((res, rej) => {
            extract.on('entry', (header, readStream, next) => {
                if (filter(header.name)) {
                    const writestream = (0, fs_1.createWriteStream)(extractPath, { mode: 0o775 });
                    const hashing = md5PassThrough();
                    digest = hashing.digest;
                    // a failed write has to fail the extraction, otherwise the entry never ends and this hangs
                    writeDone = (0, promises_1.pipeline)(readStream, hashing.stream, writestream).catch(rej);
                }
    ...
    async extractZip(mongoDBArchive, extractPath, filter) {
        ...
        let digest;
        const zipfile = await yauzl_1.default.openPromise(mongoDBArchive, { lazyEntries: true });
        for await (const entry of zipfile.eachEntry()) {
            ...
            const hashing = md5PassThrough();
            digest = hashing.digest;
            await (0, promises_1.pipeline)(readstream, hashing.stream, writestream);
        }
        return digest?.();
    }
    /**
