...
class DryMongoBinary {
     ...
     */
    static async isBinaryCorrupted(binaryPath) {
        const binaryStat = await (0, utils_1.statPath)(binaryPath);
        if ((0, utils_1.isNullOrUndefined)(binaryStat)) {
            log(`isBinaryCorrupted: could not stat binary at "${binaryPath}", treating as corrupted`);
            return true;
        }
        if (binaryStat.size < MIN_BINARY_SIZE_BYTES) {
            log(`isBinaryCorrupted: binary at "${binaryPath}" is only "${binaryStat.size}" bytes (expected at least "${MIN_BINARY_SIZE_BYTES}"), treating as corrupted`);
            return true;
        }
        const checksumFile = `${binaryPath}.md5`;
        if (!(await (0, utils_1.pathExists)(checksumFile))) {
        ...
        if ((0, resolveConfig_1.envToBool)((0, resolveConfig_1.resolveConfig)(resolveConfig_1.ResolveConfigVariables.VALIDATE_BINARY_CHECKSUM))) {
            const [checksumFileContent, actualChecksum] = await Promise.all([
                fs_1.promises.readFile(checksumFile, 'utf-8'),
                (0, utils_1.md5FromFile)(binaryPath),
            ]);
            ...
            if (expectedChecksum !== actualChecksum) {
                log(`isBinaryCorrupted: checksum mismatch for binary at "${binaryPath}" (expected: "${expectedChecksum}", actual: "${actualChecksum}"), treating as corrupted`);
                return true;
            }
        }
        return false;
     ...
     */
    static async deleteBinary(binaryPath) {
        log(`deleteBinary: deleting ${binaryPath} + checksum`);
        await fs_1.promises.rm(binaryPath, { force: true });
        await fs_1.promises.rm(`${binaryPath}.md5`, { force: true });
    }
    /**
